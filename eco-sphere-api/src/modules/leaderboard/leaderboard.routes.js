...
const claimSchema = z.object({
  reportId: z.string(),
  action: z.enum(['self', 'community', 'map']),
  selfChoice: z.enum(['bank', 'diy']).optional(),
})

// GET /leaderboard
router.get('/', authenticate, asyncHandler(async (req, res) => {
  res.json(leaderboardService.top(Number(req.query.limit) || 20))
}))

// POST /leaderboard/claim — award points for completing a recommended action
router.post('/claim', authenticate, asyncHandler(async (req, res) => {
  const { reportId, action, selfChoice } = claimSchema.parse(req.body)
  const report = repos.reports.get(reportId)
  if (!report) throw ApiError.notFound('Report not found')

  const points = pointsFor(action, selfChoice)
  const result = awardPoints(req.user.id, points, `action:${action}`, reportId)
